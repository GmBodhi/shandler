...
class SHClient extends EventEmitter {
    ...
    constructor(client, options = {}) {
        super();
        let { commandsDir = "", showLogs = "extra", autoDelete = true, cLogs = false, wrapper = false, autoRegister = true, sync = true, exclude = [] } = options;
        ...
        this.client.on("ready", async () => {
            ...
            if (!wrapper) {
                ...
                const cmdfls = fs.readdirSync(path.resolve(process.cwd(), commandsDir)).filter((m) => m.endsWith(".js") && !exclude.includes(m.replace(/(.+)\.\w?js$/, "$1")));
            ...
            if (interaction.type == 2) {
                ...
                this.client.emit(
                    ...
                    new Interaction(interaction, {
                        client: this.client,
                        guild: guild,
                        member: interaction.member ? guild?.members.add(interaction.member) : null,
                        user: interaction.user ? this.client.users.add(interaction.user) : interaction.member?.user ? this.client.users.add(interaction.member?.user) : null,
                        channel: this.client.channels.resolve(interaction.channel_id) ?? null,
                    })
                );
            } else return;
        });
