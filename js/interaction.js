...
class Interaction {
    ...
    async reply(res, options = {}) {
        ...
        let { embed, embeds = [], flags = null, type = 4, tts = false, components = [] } = options;
        ...
        let b = await this.client.api.interactions(this.id, this.token).callback.post({
            data: {
                type: type,
                data: data,
            },
    ...
    async update(res, options = {}) {
        if (this.isCommand()) throw new Error("This method is only allowed for Component interactions");
        options["type"] = 7;
